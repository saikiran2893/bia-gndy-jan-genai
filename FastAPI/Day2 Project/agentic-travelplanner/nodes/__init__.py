...
from utils.prompts import SYSTEM_PROMPT, REVISION_SYSTEM_PROMPT,_APPROVAL_SYSTEM_PROMPT
...
def human_review_node(state:TravelState)->dict:
    ...
    print(f"\n[Human Feedback]: {feedback_clean}\n")

    # is_approved = any(tok in feedback_clean.lower() for tok in _APPROVAL_TOKENS)
    response = _llm.invoke([SystemMessage(content=_APPROVAL_SYSTEM_PROMPT),HumanMessage(content=feedback_clean)])

    print(f"\n[Human Feedback]: {response}\n")

    if "APPROVE" in response.content.strip().upper():
        is_approved = True
